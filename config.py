...
from calibre_plugins.CBDB.common_utils import ReadOnlyTableWidgetItem
...
plugin_prefs = JSONConfig('plugins/CBDB')
...
class GenreTagMappingsTableWidget(QTableWidget):
    ...
    def populate_table(self, tag_mappings):
        ...
        header_labels = ['CBDB Genre', 'Maps to Calibre Tag(s)']
...
class ConfigWidget(DefaultConfigWidget):
    ...
    def __init__(self, plugin):
        ...
        self.all_authors_checkbox.setToolTip('CBDB for some books will list all of the contributing authors and\n'
    ...
    def add_mapping(self):
        new_genre_name, ok = QInputDialog.getText(self, 'Add new mapping',
                    'Enter a CBDB genre name to create a mapping for:', text='')
        if not ok:
            # Operation cancelled
            return
    ...
    def rename_genre(self):
        ...
        if not selected_genre:
            return
        new_genre_name, ok = QInputDialog.getText(self, 'Add new mapping',
                    'Enter a CBDB genre name to create a mapping for:', text=selected_genre)
        if not ok:
            # Operation cancelled
            return
        new_genre_name = unicode(new_genre_name).strip()
